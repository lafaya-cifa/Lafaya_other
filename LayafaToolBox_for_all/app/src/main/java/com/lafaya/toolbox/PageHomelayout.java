...
import static com.lafaya.toolbox.DoorStatus.TYPE_SWITCH;
...
//    private boolean light_on_flage = false,door_connect_flage = false,lock_on_flage = false;
    ...
    private TextView text_home_mode,text_home_mode_title;
    ...
    private RelativeLayout homelayout_doorinfo;
    private LinearLayout homelayout_errocode;
    ...
    public void activityHomelayout(Activity inactivity, Handler inhandler){
        ...
        text_home_mode_title = (TextView)activity.findViewById(R.id.text_home_mode_title);
        ...
        homelayout_errocode = (LinearLayout)activity.findViewById(R.id.homelayout_errocode);
        homelayout_doorinfo = (RelativeLayout)activity.findViewById(R.id.homelayout_doorinfo);
    ...
    public void UpdateHomelayout(){
        if((home_check_flag) && (home_check_list == 1)){
            return;
        }else {
            if (MainActivity.doorStatus.doortype == TYPE_REVOLVINGDOOR) {
                ...
                text_home_mode_title.setText("当前运行模式");
                homelayout_doorinfo.setVisibility(View.VISIBLE);
                homelayout_errocode.setVisibility(View.VISIBLE);

            } else if (MainActivity.doorStatus.doortype == TYPE_SWITCH) {
                //
                image_home_mode.setImageResource(R.drawable.icon_big_mode_manual);
                text_home_mode.setVisibility(View.GONE);
                button_home_light.setVisibility(View.GONE);
                button_home_lock.setVisibility(View.GONE);
                text_home_mode_title.setText("轻触开门");

                homelayout_doorinfo.setVisibility(View.GONE);
                homelayout_errocode.setVisibility(View.GONE);
    ...
    public void CheckStatusStart(final Handler handler){
        ...
        if(MainActivity.doorStatus.doortype == TYPE_SWITCH){
            if(!MainActivity.doorStatus.switch_comminication_flag){
                MainActivity.doorStatus.switch_comminication_flag = true;
                MainActivity.bluetoothComm.SendMessage(MainActivity.bluetoothComm.commandLafaya.SendLafayaQueryInit(CommandLafaya.switchsend_ID),activity);
            }else{
...
/*
    ...
    public void CleanCheckstatus(){
        ...
        if(MainActivity.doorStatus.switch_comminication_flag){
            MainActivity.doorStatus.switch_comminication_flag = false;
        }
