...
public class DoorStatus {
    ...
    public final static int TYPE_REVOLVINGDOOR = 0;
    public final static int TYPE_SLIDINGDOOR = 1;
    public final static int TYPE_SWITCH = 2;

    public int doortype = TYPE_REVOLVINGDOOR; // 0 = sliding / swing door / folding door, 1 = revolving door, 2 = switch
    ...
    public boolean switch_comminication_flag = false;
    ...
    public void errornowQuery(Activity activity){
        if(doortype == TYPE_REVOLVINGDOOR){
            if(plc_communication_flag == PLC_COMMUNICATION_FREE){
...
//open the door
    public void opendoor(Activity activity){
        ...
        }else if(doortype == TYPE_SWITCH) {
             MainActivity.bluetoothComm.SendMessage(MainActivity.bluetoothComm.commandLafaya.SendLafayaOpendoor(CommandLafaya.switchsend_ID),activity);
        }else{
